...
class BioscopeCorpus:
	""" 
	Corpus Bioscope: contiene toda la información que se levanta a memoria del corpus. La relaciÃ³n con los archivos originales se hace a travÃ©s de la instancia de BioscopeCorpus Processor (bcp) asociada
	...
	def __init__(self,bcp,prefix):
		...
		@arg bcp: ConfiguraciÃ³n del ambiente para obtener los documentos originales.
		@type bcp: L{bioscope.util.BioscopeCorpusProcessor}
		@rtype: C{None}
		@arg prefix: PatrÃ³n a buscar al comienzo del nombre del documento
		@type prefix: String
		"""
...
class BioscopeDocument:
	""" 
	Documento de Bioscope, incluyendo toda la informaciÃ³n de parsing y tagging 
	...
	def __init__(self, docId,bcp):
		""" 
		Crea el documento y las instancias de las oraciones que lo componen, levantando la informaciÃ³n de los archivos correspondientes. NO levanta la informaciÃ³n de Genia, 
		...
		# Las oraciones del corpus son un diccionario indexado por el identificador de la oraciÃ³n
		self.sentences=dict()
		i=0
		for sentenceId in sentence_ids:
			#print >> stderr,  "Levanto la oraciÃ³n:",sentenceId
	...
	def add_genia_and_bioscope_info(self,bcp):	
		"""
		Agrega al Ã¡rbol de parsing original de cada oraciÃ³n la informaciÃ³n obtenida a partir del tagger Genia
		@rtype: C{None}
		"""
		...
		for (key,sentence) in self.sentences.iteritems():
			...
			#Levanto las palabras de la oraciÃ³n, segÃºn genia
			genia_words=bcp.get_genia_words(self.docId,sentence.sentenceId)
			
			#Levanto las palabras de la oraciÃ³n, segÃºn bioscope
			...
			# Si la tokenizacion de bioscope difiere de la de Genia, intento retokenizar con unos heurÃ­sticos ahÃ­
			...
			if len(genia_words)==len(bioscope_tokens)==len(parse_tree.leaves()):
				...
				for p in parse_tree.leaves():
					...
					# Tienen las mismas palabras, asÃ­ que recorro de a uno el genia y el Ã¡rbol
					tpos=parse_tree.leaf_treeposition(j)
					leaf_parent= tpos[0:len(tpos)-1]

					# Modifico el nodo del Ã¡rbol para agregar estos atributos en node
					# La propiedad node antes tenÃ­a el POS
					# Ahora pasa a ser un diccionario. Gracias, pyhton, por dejarme hacer estas cosas
					#print parse_tree[leaf_parent].label()
					...
					# DeberÃ­a tener las mismas palabras porque usÃ© el mismo tokenizador
					...
					#parse_tree[leaf_parent].node={'lemma':lemma, 'pos':pos, 'chunk':chunk, 'entity':ne, 'specCue':specCue, 'negCue':negCue,'specXcope':specXcope,'negXcope':negXcope}
...
class BioscopeSentence:
	""" Una oraciÃ³n del corpus, incluyendo toda la informaciÃ³n de parsing y tagging. Parte de un L{bioscope.BioscopeDocument}
		@ivar docId: identificador del documento que contiene la oraciÃ³n
		@type docId: C{string}
		@ivar sentenceId: identificador de la oraciÃ³n en el corpus
		@type sentenceId: C{string}
		@ivar sindex: Ã­ndice entero de la oraciÃ³n dentro del documento
		@type sindex: C{int}
		@ivar data: Ã¡rbol de anÃ¡lisis sintÃ¡ctico de la oraciÃ³n. 
		@type data: C{nltk.Tree}
		@ivar data_loaded: indicador de si ya se cargÃ³ el contenido del documento en la estructura
		@type data_loaded: C{bool}
	"""
	
	def __init__(self,docId,sentenceId,sindex,bcp):
		"""
		Crea la estructura que va a contener la oraciÃ³n, NO levanta los datos (esto se hace en la clase L{bioscope.BioscopeDocument}, 
	...
	def has_hedging(self):
		"""
		Devuelve true si la oraciÃ³n tiene alguna marca de hedging, false en caso contrario
		@rtype: C{bool}
		"""
		...
		def has_hedge_cue(s): # Esto es solamente para un nodo
			...
			@rtype: bool
			@arg s: Nodo del Ã¡rbol a evaluar si corresponde a una marca de especulaciÃ³n
		...
		def has_hedging(t):
			"""
			Dado un arbol, devuelve True si tiene alguna marca de hedging. Devuelve True si la raÃ­z corresponde a un hedge cue, o (recursivamente) si alguno de los Ã¡rboles hijos incluye una marca.
			@rtype:bool
			@arg t:
	...
	def has_negation(self):
		"""
		Devuelve true si la oraciÃ³n tiene alguna marca de negaciÃ³n, false en caso contrario
		@rtype: C{bool}
		"""
		...
		def has_negation_cue(s): # Esto es solamente para un nodo
			...
			@rtype: bool
			@arg s: Nodo del Ã¡rbol a evaluar si corresponde a una marca de negaciÃ³n
			@type s: types.StringType o types.DictType
			"""
			if isinstance(s,types.StringType):
				return False
		...
		def has_negation(t):
			"""
			Dado un arbol, devuelve True si tiene alguna marca de negation. Devuelve True si la raÃ­z corresponde a un negation cue, o (recursivamente) si alguno de 
			los Ã¡rboles hijos incluye una marca.
			@rtype:bool
			@arg t:
			@type t: nltk.tree.Tree
			"""
	...
	def get_dot(self):
		"""
		Devuelve un string en formato graphviz que representa el Ã¡rbol de la oraciÃ³n (incluyendo las marcas de tagging).
		@rtype: C{string}
		"""
		...
		def gv_print(t,inicial=1):
			"""
			Dado un arbol, genera una representaciÃ³n dot
			comenzando la numeraciÃ³n de los nodos en el valor dado por el pÃ¡rametro inicial
			"""
			
			# Show the tree's root node 
			s='%s [label="%s"]' % (inicial,gv_rep(t.label()))
	...
	def get_basic_attributes(self):
		"""
		Devuelve la lista de tokens de la oraciÃ³n junto con sus atributos.
		...
		def get_tree_leaves(t):
			...
			# AhÃ­ voy a tener todos los atributos
			# Sino simplemente hago recursiÃ³n
			res=[]
			for child in t: 
	...
	def get_leaf_grandparent(self,leaf_pos,gp_number):
		"""
		Dada una hoja del Ã¡rbol, devuelve Ã©l arbol de su abuelo y una especificaciÃ³n de la posiciÃ³n en el Ã¡rbol original, vÃ­a un treepos
		@arg leaf_pos: NÃºmero de hoja
		@type leaf_pos: L{int}
		@arg gp_number: nÃºmero de grandparent que es. Abuelo=2, Bisabuelo=3, etc.
		@rtype: C{nltk.Tree}		
		...
		# El de su abuelo es el mismo, sin los gp_number Ãºltimos pasos
	...
	def get_node_scope(self,treepos):
		"""
		Dado un nodo del Ã¡rbol, devuelve las posiciones de la primera y Ãºltima hoja de su alcance
		@arg treepos: EspecificaciÃ³n de la posiciÃ³n del nodo en el Ã¡rbol de la oraciÃ³n
		@type treepos: C{List}
		@rtype: C{sequence}
		"""
		parse_tree=self.data
	...
	def get_node_hedge_scope(self,treepos):
		...
		para que ajuste mÃ¡s a los criterios de bioscope
		@arg treepos: EspecificaciÃ³n de la posiciÃ³n del nodo en el Ã¡rbol de la oraciÃ³n
		@type treepos: C{List}
		@rtype: C{sequence}
		"""
			...
			for child in parse_tree[treepos]:
				if isinstance(child.node,types.DictType):
					...
					if child.node not in ['ADVP','PP']:
						...
						#print "Encontré un ejemplo de ADVP o PP a la izquierda en la oración "
						pass
				child_number+=1

		# Si es un NP, elimino los determinantes al principio, si estÃ¡n antes de un adjetivo
		# Aparece en la corrida 37
		# En la corrida 40 esto se cambiÃ³ por una regla de postprocesamiento
			...
			while j<len(children):
				...
				if isinstance(child.node,types.DictType):
					if child.node['pos']=='DT' and children[j+1].node['pos']=='JJ':
						...
						#print "Encontré un ejemplo de determinante antes de adjetivo a la izquierda "
	...
	def get_scope_start(self,treepos):
		"""
		Dado un Ã¡rbol y un nodo dado por una posiciÃ³n en el Ã¡rbol, devuelve el comienzo del alcance del nodo
		"""
	...
	def get_scope_end(self,treepos):
			"""
			Dado un Ã¡rbol y un nodo dado por una posiciÃ³n en el Ã¡rbol, devuelve el final del alcance del nodo
			"""
			parse_tree=self.data
	...
	def get_node_hedge_scope(self,treepos,hedge_cue_pos,use_heuristics):
		"""
		Dado un nodo del Ã¡rbol, devuelve las posiciones (empezando de 0) de la primera y Ãºltima hoja de su alcance
		Desde el punto de vista del hedging (lo que puede hacer que se poden algunas ramas del constituyente
		para que ajuste mÃ¡s a los criterios de bioscope. 
		TambiÃ©n utiliza la posiciÃ³n de la hedge cue en la secuencia para hacer el procesamiento
		@arg treepos: EspecificaciÃ³n de la posiciÃ³n del nodo en el Ã¡rbol de la oraciÃ³n
		@type treepos: C{List}
		@rtype: C{sequence}
		"""
		...
		if use_heuristics:
			if node_pos in ['S','SBAR','VP']:
				...
				# Si a la derecha hay algÃºn because, entonces el final es el token anterior al because
				j=hedge_cue_pos
				while j<=end:
					child=leaves[j]
					#print j,child
					if child in ['because','since','like','unlike','unless','minus','although','ie']:
						#print "EjecutÃ© regla de preposicion o CC"
				...
				# Si el Ãºltimo token es una coma, entonces la quito tambiÃ©n
				if leaves[end]==',':
					#print "ajustÃ© coma final"
					end=end-1
			elif node_pos in ['NP']:
				#print "ApareciÃ³ un node pos en NP"
